...
from src.models.utils import pixel2latent
...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.video_people_image_processor = VaeImageProcessor(
            vae_scale_factor=self.vae_scale_factor,
            do_convert_rgb=True,
            do_normalize=True,
        )
        self.simple_transform = transforms.ToTensor()
        self.resize_norm_transform = transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True)
    ...
    def __call__(
        ...
        tile_size: Optional[int] = 14,
        tile_overlap: Optional[int] = 2,
        ...
        for pose_image in pose_images:
            pose_cond_tensor = self.pose_image_processor.preprocess(
        ...
        pose_latents = self.pose_guider(pose_cond_tensor).transpose(
            1, 2
        ) # (bs, f, c, H, W)  
        pose_latents = pose_latents.repeat(2 if do_classifier_free_guidance else 1, 1, 1, 1, 1)  

        # prepare agnostic
        video_length = len(agnostic_list)
        agn_tensor_list = []
        for agn in agnostic_list:
            agn_tensor = self.ref_image_processor.preprocess(
                agn, height=height, width=width
            )      
            
            agn_tensor_list.append(agn_tensor)
        agn_tensor = torch.cat(agn_tensor_list, dim=0)
        agn_tensor = agn_tensor.to(dtype=self.vae.dtype, device=self.vae.device)
        ...
        agnostic_image_latents = rearrange(agnostic_image_latents, "(b f) c h w -> b f c h w", f=video_length)
        
        
        # Prepare a list of mask condition images
        agn_mask_list = []        
        for mask in video_mask_image_list:
            mask = self.ref_image_processor.preprocess(
                mask, height=height, width=width
            )  
            mask = self.ref_image_processor.normalize(mask)
        ...
        agn_mask = rearrange(agn_mask, "(b f) c h w -> b f c h w", f=video_length)
        
        latents = latents.to(dtype=self.unet.dtype)
        agnostic_image_latents = agnostic_image_latents.to(dtype=self.unet.dtype)
        agn_mask = agn_mask.to(dtype=self.unet.dtype, device=latents.device)
        agnostic_image_latents = torch.cat([agnostic_image_latents] * 2) if do_classifier_free_guidance else agnostic_image_latents
        agn_mask = torch.cat([agn_mask] * 2) if do_classifier_free_guidance else agn_mask
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latent_model_input, agnostic_image_latents, agn_mask], dim=2)
                ...
                for batch, index_start in enumerate(range(0, num_frames, tile_size)):
                    self.scheduler._step_index = None
                    index_start -= shift
                    end_idx=min(index_start+tile_size, pose_latents.shape[1])
                    idx = list(range(index_start, end_idx))
                    _noise_pred = self.unet(
                        latent_model_input[:, idx],
                        t,
                        encoder_hidden_states=image_embeddings,
                        added_time_ids=added_time_ids,
                        pose_cond_fea=pose_latents[:, idx],
                        return_dict=False,
                    )[0]
 
                    noise_pred[:, idx] += _noise_pred
                    noise_pred_cnt[idx] += 1
                ...
                # latents = self.scheduler.step(noise_pred, t, latents, noise_type="video_fusion").prev_sample
