...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        indices = [[*range(i, min(i + tile_size, num_frames))] for i in
                   range(0, num_frames - tile_size, tile_size - tile_overlap)]
        if indices[-1][-1] < num_frames - 1:
            indices.append([*range(num_frames - tile_size, num_frames)])
